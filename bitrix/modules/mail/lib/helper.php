...
class Helper
{
	...
	protected static function syncImapMailbox($mailbox, &$error)
	{
		...
		$nouid = \Bitrix\Main\Config\Option::get('mail', sprintf('imap_mailbox_no_uid_%u', $mailbox['ID']), 'N');
		...
		foreach (array_merge($imapOptions['income'], $imapOptions['outcome']) as $name)
		{
			...
			foreach ($list as $item)
			{
				...
				else if ($nouid != 'Y')
				{
					addMessage2Log(
						sprintf(
							'IMAP: UID not found (%s:%s/%s)',
							$mailbox['SERVER'], $mailbox['PORT'], $mailbox['LOGIN']
						),
						'mail', 0, false
					);

					\Bitrix\Main\Config\Option::set('mail', sprintf('imap_mailbox_no_uid_%u', $mailbox['ID']), $nouid = 'Y');
				}
