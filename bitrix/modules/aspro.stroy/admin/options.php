...
if($mid){
	if($RIGHT >= "R"){
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && $RIGHT >= "W" && check_bitrix_sessid()){
			...
			$compositeMode = $moduleClass::IsCompositeEnabled();
							...
							if(array_key_exists($optionCode, $arTab["OPTIONS"])){
								...
								if(isset($arOption["NOTE"])){
									if($optionCode == 'GOALS_NOTE'){
										$FORMS_GOALS_LIST = '';
										if(CCache::$arIBlocks[$optionsSiteID]['aspro_stroy_form'] && is_array(CCache::$arIBlocks[$optionsSiteID]['aspro_stroy_form'])){
											foreach(CCache::$arIBlocks[$optionsSiteID]['aspro_stroy_form'] as $arIDs){
