...
class CSocServFacebook extends CSocServAuth
{
	...
	public function getUrl($arParams)
	{
		...
		{
			$redirect_uri = static::CONTROLLER_URL."/redirect2.php";
...
<?
	...
	public function getFriendsList($limit, &$next)
	{
		if(IsModuleInstalled('bitrix24') && defined('BX24_HOST_NAME'))
		{
			$redirect_uri = self::CONTROLLER_URL."/redirect2.php?redirect_to=".urlencode(CSocServUtil::GetCurUrl('auth_service_id='.self::ID, array("code")));
		}
		else
		{
