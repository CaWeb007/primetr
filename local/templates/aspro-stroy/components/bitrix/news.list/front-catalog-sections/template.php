			...
			<?foreach($arResult['SECTIONS'] as $arItem):
				...
				<div class="col-md-<?=$colmd?> col-sm-<?=$colsm?> col-xs-<?=$colxs?>">
					<div class="item<?=($bSectionImage ? '' : ' wti')?> wline" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<a title="<?=$arItem["~NAME"]?>" href="<?=$arItem['SECTION_PAGE_URL']?>" class="dark_block_animate"></a>
							...
							<?if(in_array('NAME', $arParams['FIELD_CODE'])):?>
								<div class="title">
									<span class="wrap_outer titles">
										<a title="<?=strip_tags($arItem["~NAME"])?>" href="<?=$arItem['SECTION_PAGE_URL']?>" class="outer_text">
