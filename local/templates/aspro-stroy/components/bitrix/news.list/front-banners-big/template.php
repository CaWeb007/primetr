					...
					<?foreach($arResult['ITEMS'] as $i => $arItem):?>
						...
						<li class="item" id="<?=$this->GetEditAreaId($arItem['ID'])?>" style="background:url('<?/*=$imageBgSrc*/?>') center center no-repeat !important;">
							<a href="<?=$arItem['PROPERTIES']['LINKIMG']['VALUE']?>">
								<img style="display: block;width: 100%;height:auto;margin: 0 auto;padding:0;" src="<?=$imageBgSrc?>" />
							</a>
							<div class="maxwidth-theme<?=($bOnlyImage && $bLinkOnName ? ' fulla' : '')?>" style="height:100%;">
								<div class="row <?=$arItem['PROPERTIES']['TEXTCOLOR']['VALUE_XML_ID']?> <?=($type != 'T2' ? 'righttext' : '')?>" style="height:100%;display:flex;align-items:center;">
									<?ob_start();?>
									<?if(!$bOnlyImage):?>
										<?if($bLinkOnName):?>
										...
										<div class="detail-text-block" style="margin: -10px auto 0;color: #fff;font-size: 36px;line-height: 38px;padding: 0 0 9px 0;font-weight: bold;"><?=$arItem['DETAIL_TEXT']?></div>
