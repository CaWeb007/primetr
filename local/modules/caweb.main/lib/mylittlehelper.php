...
class MyLittleHelper {
    ...
    */
    ...
    //title: delete necondition order items
    \Bitrix\Main\Loader::includeModule('caweb.main');
    \Caweb\Main\MyLittleHelper::deleteItemsFile();
     */
    public static function deleteItemsFile(){
        $exept = array(155, 80, 79, 55, 54, 56, 53, 81, 82, 83, 145, 146, 147, 148, 149, 150, 151, 153, 154, 156, 157, 52, 158, 159, 51);
        Loader::includeModule('iblock');
        $iblockId = 25;
        $el = new \CIBlockElement;
        $db = \CIBlockElement::GetList(array(), array('IBLOCK_ID' => $iblockId, 'SECTION_ID' => 269, 'INCLUDE_SUBSECTIONS' => 'Y'));
        while ($ar = $db->GetNextElement()){
            $fields = $ar->GetFields();
            $props = $ar->GetProperties(array(), array('PROPERTY_TYPE' => 'F'));
            $updateFields = array();
            $updateProps = array();
            if (!empty($fields['PREVIEW_PICTURE']))
                $updateFields['PREVIEW_PICTURE'] = array('del' => 'Y');
            if (!empty($fields['DETAIL_PICTURE']))
                $updateFields['DETAIL_PICTURE'] = array('del' => 'Y');
            foreach ($props as $code => $item){
                if (!empty($item['VALUE']))
                    $updateProps[$code] = array('VALUE' => array('del' => 'Y'));
            }
            if (in_array((int)$fields['ID'], $exept))
                $updateFields = array();
            if (empty($updateProps) && empty($updateFields)) continue;
            if (!empty($updateFields))
                $el->Update($fields['ID'], $updateFields);
            if (!empty($updateProps))
                $el::SetPropertyValuesEx($fields['ID'], $iblockId, $updateProps);
        }
